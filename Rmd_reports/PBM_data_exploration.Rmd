...
metadata[, c("norm_method", "experiment")] <- t(as.data.frame(strsplit(x = metadata$dataset, split = "_")))
...
for (experiment in experiments) {
  dataset <- paste(sep="_", norm_method, experiment) 
...
for (experiment in experiments) {
  ...
  dataset <- paste(sep = "_", method, experiment)
  TF <- unique(metadata[metadata$dataset == dataset, "TF"])
  ...
  # hist(signal_log10, 
  #      breaks=breaks_log10, 
  #      main=paste(TF, method, experiment, "\nlog10(signal)"))
